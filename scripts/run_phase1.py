#!/usr/bin/env python3
"""Run the configured Phase 1 frozen-feature prediction-head sweep."""
...
from nbv.experiments.phase1 import (  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/phase1_sweep.yaml"),
    )
    parser.add_argument(
        "--set",
        action="append",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        completed_dir = run_phase1_sweep(
            config, REPOSITORY_ROOT, logger=logger
        )
    except (ConfigError, KeyError, TypeError, ValueError, RuntimeError) as exc:
        logger.error("Phase 1 sweep failed: %s", exc)
        return 1
    logger.info("Comparison table: %s", completed_dir / "metrics/comparison.csv")
    return 0
