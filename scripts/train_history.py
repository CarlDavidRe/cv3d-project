#!/usr/bin/env python3
"""Train the Phase 3 independent direct surface-gain history control."""
...
from nbv.experiments.phase3_independent import (  # noqa: E402
    parse_phase3_independent_settings,
    run_phase3_independent,
)
from nbv.features import FeatureCacheError  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/phase3_independent.yaml"),
    )
    parser.add_argument(
        "--set",
        action="append",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        settings = parse_phase3_independent_settings(config, REPOSITORY_ROOT)
    ...
    logger.info(
        "Phase 3 independent control: %s aggregation on %s",
        "masked_mean",
        settings.device,
    )
    try:
        completed = run_phase3_independent(config, REPOSITORY_ROOT, logger=logger)
    except (
        FeatureCacheError,
        HistoryDatasetError,
        HistoryFeatureError,
        ...
        logger.error("Phase 3 independent training failed: %s", exc)
        return 1
